...
class MypageScrapedRecipePagingSource @Inject constructor(
    private val mypageApi: MypageApi
) : PagingSource<Int, ScrapedRecipeContent>() {

    // 이 메서드는 데이터 로드의 핵심 부분으로, 비동기로 호출되어 데이터를 가져옵니다.
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ScrapedRecipeContent> {
        val page = params.key ?: 0 // 초기 페이지 키가 없으면 0으로 설정합니다.

        return try {
            // API를 호출하여 데이터를 가져옵니다.
            val response =
                mypageApi.getMyScrapedRecipeList(page, params.loadSize)
    ...
    override fun getRefreshKey(state: PagingState<Int, ScrapedRecipeContent>): Int? {
