...
import com.project.veganlife.mypage.data.datasource.MypageWrotedCommentPagingSource
import com.project.veganlife.mypage.data.datasource.MypageWrotedFeedPagingSource
...
import com.project.veganlife.mypage.data.datasource.MypageWrotedRecipePagingSource
...
class MypageRepositoryImpl @Inject constructor(
    ...
    override suspend fun getMyWroted(type: String): Flow<PagingData<MyPostedContent>> {
        return when(type) {
            "feed" ->
                Pager(
                    config = PagingConfig(pageSize = 20, enablePlaceholders = false),
                    pagingSourceFactory = {
                        MypageWrotedFeedPagingSource(mypageApi) }
                ).flow // Flow로 변환하여 반환합니다.

            else ->
                Pager(
                    config = PagingConfig(pageSize = 20, enablePlaceholders = false),
                    pagingSourceFactory = { MypageWrotedCommentPagingSource(mypageApi) }
                ).flow // Flow로 변환하여 반환합니다.
        }
    }

    override suspend fun getScrapedRecipe(): Flow<PagingData<RecipeFeedContent>> {
        return Pager(
            config = PagingConfig(pageSize = 20, enablePlaceholders = false),
            pagingSourceFactory = {
    ...
    override suspend fun getWrotedRecipe(): Flow<PagingData<RecipeFeedContent>> {
        return Pager(
            config = PagingConfig(pageSize = 20, enablePlaceholders = false),
            pagingSourceFactory = {
                MypageWrotedRecipePagingSource(
                    mypageApi,
                )
            }
        ).flow // Flow로 변환하여 반환합니다.
